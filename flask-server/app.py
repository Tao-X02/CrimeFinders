...
def get_matches():
    ...
    post = requests.get('http://localhost:5000/api/posts/' + post_id) # Change to http://host.docker.internal:5000 if running on docker
    post = post.json()
    ...
    if len(photos) > 0:
        ...
        if len(face_encodings) > 0:
            ...
            all_posts = requests.get('http://localhost:5000/api/posts') # Change to http://host.docker.internal:5000 if running on docker
            p = all_posts.json()
